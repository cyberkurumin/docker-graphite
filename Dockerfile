...
ADD http://grafanarel.s3.amazonaws.com/grafana-1.6.0.tar.gz /var/lib/
RUN cd /var/lib/ && \
    tar -xvzf /var/lib/grafana-1.6.0.tar.gz && \
    mv grafana-1.6.0 grafana && \
    rm -rf /var/lib/grafana-1.6.0.tar.gz
ADD ./config.js /var/lib/grafana/config.js
